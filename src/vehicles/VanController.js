                            ...
                            const rack = MeshBuilder.CreateBox('van_rack', { width: 2.0, height: 0.08, depth: 4.6 }, this.scene);
                                rack.parent = body;
                                    rack.position.y = 0.94;
                                        ...
                                        this.wheels = [
                                                  new Vector3(-1.2, -0.6, 1.6), new Vector3(1.2, -0.6, 1.6),
                                                        new Vector3(-1.2, -0.6, -1.6), new Vector3(1.2, -0.6, -1.6),
                                        ].map((pos, i) => {
                    ...
                    window.addEventListener('keydown', e => { keys[e.key] = true; });
                        window.addEventListener('keyup', e => { keys[e.key] = false; });
                            this.scene.onBeforeRenderObservable.add(() => {
                                      this.input.forward = (keys['ArrowUp'] || keys['w'] || keys['W']) ? 1 : 0;
                                            this.input.backward = (keys['ArrowDown'] || keys['s'] || keys['S']) ? 1 : 0;
                                                  this.input.left = (keys['ArrowLeft'] || keys['a'] || keys['A']) ? 1 : 0;
                                                        this.input.right = (keys['ArrowRight'] || keys['d'] || keys['D']) ? 1 : 0;
                                                              this.input.honk = keys['h'] || keys['H'];
                            });
          }
                                        ...
                                        osc.frequency.setValueAtTime(320, this._audioCtx.currentTime);
                                            gain.gain.setValueAtTime(0.3, this._audioCtx.currentTime);
                                                gain.gain.exponentialRampToValueAtTime(0.001, this._audioCtx.currentTime + 0.5);
                                                    osc.start(); osc.stop(this._audioCtx.currentTime + 0.5);
                                                        setTimeout(() => { this._hornPlaying = false; }, 600);
              }

                update(deltaTime) {
                        const dt = deltaTime / 1000;
                            const fwd = this.input.forward || (window.touchInput?.y < -0.2 ? -window.touchInput.y : 0);
                                const bwd = this.input.backward || (window.touchInput?.y > 0.2 ? window.touchInput.y : 0);
                                    const lft = this.input.left || (window.touchInput?.x < -0.2 ? -window.touchInput.x : 0);
                                        const rgt = this.input.right || (window.touchInput?.x > 0.2 ? window.touchInput.x : 0);

                                            if (fwd) this.speed = Math.min(this.maxSpeed, this.speed + this.acceleration * dt);
                                                else if (bwd) this.speed = Math.max(-this.maxSpeed * 0.4, this.speed - this.braking * dt);
                                                    else { this.speed *= this.friction; if (Math.abs(this.speed) < 0.1) this.speed = 0; }
                                                                            ...
                                                                            this.mesh.rotation.y += this.steerAngle * Math.sign(this.speed);
                                                                                this.mesh.position.addInPlace(forward.scale(this.speed * dt));
                                                                                    ...
                                                                                    if (this.terrain) {
                                                                                        ...
                                                                                        this.wheels?.forEach(w => { w.rotation.x += (this.speed / 20) * dt; });
                                                                                            if (this.input.honk || window.touchInput?.honk) this._honk();
                }

                  getPosition() { return this.mesh.position.clone(); }
                    setColor(r, g, b) { if (this.mesh?.material) this.mesh.material.diffuseColor = new Color3(r, g, b); }
}
                                                                                    }
                }
              }
